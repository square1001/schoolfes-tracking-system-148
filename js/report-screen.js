...
// ---------- UPDATE ROOM STATUS ---------- //
function tostring_fillzero(x, num) {
	var res = String(x);
	...
	simpleinforef.once("value", function(snapshot_info) {
		roomstateref.once("value", function(snapshot) {
			...
			for(var i = 0; i < valid_place.length; ++i) {
				var id_str = tostring_fillzero(i, 2);
			...
			var dt = new Date();
			var timestr = dt.getFullYear() + "/" + dt.getMonth() + "/" + dt.getDate() + " " + dt.getHours() + ":" + tostring_fillzero(dt.getMinutes(), 2) + ":" + tostring_fillzero(dt.getSeconds(), 2);
			document.getElementById("last-update").innerHTML = "(" + timestr + " 更新)";
		});
	});
}
setInterval("update_room_state_table()", 120 * 1000);